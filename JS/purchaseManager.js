        ...
        switch(type){
            case 0:
                if(this.battle.players==1){
                    let list=variants.ultraprism?[types.card,types.card,types.card,types.card]:variants.prism?copyArrayStack(this.battle.cardManagers[0].listing.allPlayerCard):copyArrayStack(this.battle.cardManagers[0].listing.card[this.battle.player[0]])
                    ...
                    for(let a=0,la=group.length;a<la;a++){
                        ...
                        this.purchases.push(new purchase(this.layer,this.battle,0,100+a%4*150,112.5+floor(a/4)*187.5,1,[round(random(cost[group[a]][0],cost[group[a]][1]))],[variants.ultraprism?index:list[group[a]][index],0,variants.ultraprism?(types.card[index].list<0?0:types.card[index].list>=types.color.length?0:types.card[index].list):variants.prism?types.card[list[group[a]][index]].list:this.battle.player[0]]))
                    ...
                    for(let a=0,la=this.battle.players;a<la;a++){
                        let list=variants.ultraprism?[types.card,types.card,types.card,types.card]:variants.prism?copyArrayStack(this.battle.cardManagers[a].listing.allPlayerCard):copyArrayStack(this.battle.cardManagers[a].listing.card[this.battle.player[a]])
                        let group=[0,0,0,0,1,1,1,2]
                        ...
                        for(let b=0,lb=group.length;b<lb;b++){
                            ...
                            this.purchases.push(new purchase(this.layer,this.battle,a,this.layer.width*a+(100+(b%4*100))*(1-a*2),350+floor(b/4)*150,1,[price,price],[variants.ultraprism?index:list[group[b]][index],0,variants.ultraprism?(types.card[index].list<0?0:types.card[index].list>=types.color.length?0:types.card[index].list):variants.prism?types.card[list[group[b]][index]].list:this.battle.player[a]]))
