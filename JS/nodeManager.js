    ...
    setupMap(){
        ...
        for(let a=0,la=20;a<la;a++){
            for(let b=0,lb=min(min(a+1,4),20-a);b<lb;b++){
                this.nodes.push(new node(this.layer,this.battle,this.layer.width/2+60-lb*60+b*120,this.layer.height/2+a*100-150-min(3,a)*10,b,a,a<2?1:a==la-1?2:possibilities[floor(random(0,possibilities.length))]))
            }
        }
