            ...
            if(this.userCombatant.life<=0){
                ...
                switch(this.action){
                    case 0:
                        if(this.attackClass==2||this.attackClass==4){
                            ...
                            switch(this.type){
                                ...
                                case 436: case 439: case 443: case 445: case 448: case 454: case 455: case 459: case 463:
                                    this.target=[
                                        [this.userCombatant.tilePosition.x+transformBase[0],this.userCombatant.tilePosition.y+transformBase[1]],
                                        [this.userCombatant.tilePosition.x+transformBase[0]*2,this.userCombatant.tilePosition.y+transformBase[1]*2],
                                        [this.userCombatant.tilePosition.x+transformBase[0]*3,this.userCombatant.tilePosition.y+transformBase[1]*3],
                            ...
                            if(
                            ...
                            }else if(this.type==429||this.type==430){
                                if(this.targetIndex[1]==-1){
                                    if(this.mover){
                                        this.userCombatant.moved=true
                                    }
                                    this.remove=true
                                }else{
                                    this.targetCombatant=this.battle.combatantManager.combatants[this.targetIndex[1]]
                                ...
                                this.type==395||this.type==404||this.type==409||this.type==417||this.type==418||this.type==441||this.type==449||this.type==468||this.type==470
                            ){
                                ...
                                for(let a=0,la=this.targetIndex.length;a<la;a++){
                                    ...
                                    switch(this.type){
                                        ...
                                        break
                                        case 49: case 131: case 164: case 185: case 195: case 205: case 219: case 286: case 384: case 406:
                                        case 444:
                                            if(
                    ...
                    case 1:
                        ...
                        switch(this.type){
                            case 0: case 4: case 6: case 8: case 9: case 15:
                                let works=true
                                if(this.setTarget!=-1){
                                    ...
                                    if(this.type==15){
                                        ...
                                        this.target=result.length>=1?[result[0]]:[this.battle.combatantManager.getPlayerCombatantIndex(this.userCombatant.target)]
                                    }else{
                            ...
                            case 14:
                                ...
                                if(works14){
                                    this.possible=[]
                                    for(let a=0,la=6;a<la;a++){
                                        let index=this.battle.tileManager.getTileIndex(this.target.tilePosition.x+transformDirection(0,360*(a+0.5)/la)[0]*2,this.target.tilePosition.y+transformDirection(0,360*(a+0.5)/la)[1]*2)
                                        if(index>=0&&this.battle.tileManager.tiles[index].occupied==0){
                                            this.possible.push(index)
                                        }
                                    }
                                    this.movable=[]
                                    for(let a=0,la=this.possible.length;a<la;a++){
                                        if(distTarget(0,this.battle.tileManager.tiles[this.possible[a]].tilePosition.x-this.userCombatant.tilePosition.x,this.battle.tileManager.tiles[this.possible[a]].tilePosition.y-this.userCombatant.tilePosition.y)==1){
                                            this.movable.push(this.possible[a])
                                        }
                                    }
                                    if(this.movable.length>0){
                                        ...
                                        this.relativeDirection=atan2(this.targetTile.relativePosition.x-this.relativePosition.x,this.targetTile.relativePosition.y-this.relativePosition.y)
                                        this.relativeDistance=sqrt((this.targetTile.relativePosition.x-this.relativePosition.x)**2+(this.targetTile.relativePosition.y-this.relativePosition.y)**2)
                                    }else if(!legalTargetCombatant(0,2,2,this.target,this.userCombatant,this.battle.tileManager.tiles)){
                                        this.type=0
                                        this.set()
                                    }else{
                                        this.remove=true
        ...
        switch(type){
            case 0:
                switch(this.type){
                    ...
                    break
                    case 426:
                        this.targetCombatant.takeDamage(this.effect[0]*this.userCombatant.getStatus('Armor'),this.user)
                    break
                    case 434: case 435: case 448:
                        this.targetCombatant.statusEffect('Weak Next Turn',this.effect[0])
                        this.targetCombatant.statusEffect('Vulnerable Next Turn',this.effect[0])
                        this.targetCombatant.statusEffect('Frail Next Turn',this.effect[0])
                    break
                ...
                switch(this.type){
                    ...
                    break
                    case 454:
                        this.userCombatant.statusEffect('Weak Next Turn',this.effect[1])
                    break
                    case 455:
                        this.userCombatant.statusEffect('Vulnerable Next Turn',this.effect[1])
                    break
            ...
            case 1:
                switch(this.type){
                    ...
                    case 401:
                        this.battle.combatantManager.allEffect(6,[this.effect[0]])
                        this.battle.combatantManager.allEffect(9,[1])
                        this.battle.combatantManager.allEffect(48,['Strength',this.effect[1]])
                    break
                    case 410:
                        this.userCombatant.addBlock(this.effect[0]*(1+floor(random(0,2))))
                    break
                    case 442:
                        this.battle.combatantManager.allEffect(6,[this.effect[0]])
                    ...
                    default:
                        ...
                        switch(this.type){
                            ...
                            break
                            case 408:
                                this.userCombatant.statusEffectNext('Counter Shockwave',this.effect[1])
                            break
                            case 411:
                            ...
                            break
                            case 456:
                                this.userCombatant.statusEffectNext('Pity',this.effect[1])
                            break
                            case 460:
                                this.userCombatant.statusEffectNext('Regeneration',this.effect[1])
                            break
            ...
            case 2:
                switch(this.type){
                    ...
                    break
                    case 458:
                        this.battle.combatantManager.combatants[this.battle.combatantManager.getPlayerCombatantIndex(this.userCombatant.target)].statusEffect('Temporary All Cost Up Next Turn',this.effect[0])
                    break
                }
            break
            case 3:
                for(let a=0,la=this.targetCombatant.length;a<la;a++){
                    ...
                    switch(this.type){
                        ...
                        break
                        case 470:
                            this.targetCombatant[a].statusEffect('Block Down',this.effect[1])
                        break
                    }
                }
                switch(this.type){
            ...
            case 5:
                switch(this.type){
                    ...
                    break
                    case 413:
                        this.battle.combatantManager.allEffect(48,['Dodge',this.effect[0]])
                    break
                    case 421:
                        this.userCombatant.statusEffect('Chocolate Chip',this.effect[0])
                    break
                    case 423:
                        this.userCombatant.statusEffect('Radiation',this.effect[0])
                    break
                    case 432:
                        this.battle.combatantManager.allEffect(48,['Temporary Strength Next Turn',this.effect[0]])
                    break
                    case 450:
                        this.battle.tileManager.fireAreaZigzag(0,this.effect[0],this.targetCombatant.tilePosition,this.effect[1])
                    break
                    case 461:
                        this.userCombatant.statusEffect('Intangible Next Turn',this.effect[0])
